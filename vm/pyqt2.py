import sys, socket, vm, address, threading, os
...
class Window(QWidget):
	def __init__(self, parent=None):
		...
		self.setWindowTitle("VOTING MACHINE")
		self.resize(400, 300)
	...
	def votingDomain(self):
		groupBox = QGroupBox("VOTING DOMAIN")
		groupBox.setStyleSheet("QGroupBox { background-color: rgb(rgb(242, 242, 242)); border:3px solid rgb(255, 0, 0); }")
		groupBox.setStyleSheet('''
		QGroupBox {
			border: 3px solid black;
			border-radius: 5px;
		...
		''')
		qbox = QBoxLayout(2,None)
		
		hbox1 = QHBoxLayout()
		enterAdhaar = QLabel()
		enterAdhaar.setText("ENTER ADHAAR NO :")
		inputAdhaar = self.inputAdhaar = QLineEdit()
		inputAdhaar.setMaxLength(12)
		hbox1.addWidget(enterAdhaar,0,Qt.AlignCenter)
		hbox1.addWidget(inputAdhaar,0,Qt.AlignCenter)
		
		verifyAdhaar = self.verifyAdhaarBtn = QPushButton('VERIFY ADHAAR', self)
		verifyAdhaar.setToolTip('VERIFY YOUR ADHAAR NUMBER')
		verifyAdhaar.clicked.connect(self.verifyAdhaar)
		...
		partySelection = QLabel()
		partySelection.setText("CHOOSE PARTY")
		
		self.radio1 = QRadioButton("PARTY A")
		self.radio2 = QRadioButton("PARTY B")
		self.radio3 = QRadioButton("PARTY C")
		self.radio4 = QRadioButton("NOTA")
		
		# self.radio1.toggled.connect(self.clicked)
		...
		voteButton = QPushButton('CONFIRM VOTE', self)
		voteButton.setToolTip('CLICK TO CONFIRM VOTE')
		voteButton.clicked.connect(self.voteCasted)
		voteButton.setStyleSheet("QPushButton{background-color: rgb(223, 230, 233);border-width: 2px;border-radius: 10px;border-color: white;font: bold 14px;min-width: 10em;padding: 6px;}""QPushButton:pressed {background-color:rgb(178, 190, 195);}")
			
		qbox.addSpacing(15)
		qbox.addLayout(hbox1,0)
		qbox.addSpacing(15)
		qbox.addWidget(verifyAdhaar,1,Qt.AlignCenter)
		qbox.addWidget(sep,0,Qt.AlignCenter)
		qbox.addWidget(partySelection,0,Qt.AlignCenter)
		qbox.addWidget(self.radio1,0,Qt.AlignCenter)
		qbox.addWidget(self.radio2,0,Qt.AlignCenter)
		qbox.addWidget(self.radio3,0,Qt.AlignCenter)
		qbox.addWidget(self.radio4,0,Qt.AlignCenter)
		qbox.addWidget(sep2,0,Qt.AlignCenter)
		qbox.addWidget(voteButton,0,Qt.AlignCenter)
		
		
		groupBox.setLayout(qbox)
		return groupBox
	...
	def voteCasted(self):
		...
		if self.radio1.isChecked():
			party = self.radio1.text()
		elif self.radio2.isChecked():
			party = self.radio2.text()
		elif self.radio3.isChecked():
			party = self.radio3.text()
		else:
			party = self.radio4.text()
		ret = self.conn.broadcastVote(self.inputAdhaar.text(), party)

		if not ret[0]:
			# Try agn
			errorMessage = QMessageBox.warning(self, 'VOTE CAST ERROR', ret[1], QMessageBox.Retry, QMessageBox.Retry)
		else:
			# Vote casted successfully
			msg = QMessageBox.information(self, 'SUCCESSFUL VOTE', 'Your key: {}'.format(ret[2]), QMessageBox.Ok, QMessageBox.Ok)
		self.inputAdhaar.clear()
		self.inputAdhaar.setEnabled(True)
		self.verifyAdhaarBtn.setEnabled(True)
		
	def verifyAdhaar(self):
		input_aadhaar = self.inputAdhaar.text()
		self.inputAdhaar.setEnabled(False)
		ret = self.conn.verifyVoterId(input_aadhaar)
		if not ret[0]:
			# Create try window
			errorMessage = QMessageBox.critical(self, 'VOTER NOT FOUND', ret[1], QMessageBox.Close, QMessageBox.Close)
			self.inputAdhaar.clear()
			self.inputAdhaar.setEnabled(True)
		else:
			self.verifyAdhaarBtn.setEnabled(False)
	...
	def socketCreation(self):
		...
		try:
			...
			self.conn = vm.Node(s, address.Address(self.ip, self.port), address.Address(host_ip, int(host_port)))
		...
		self.resize(800, 300)
		self.grid.addWidget(self.votingDomain(), 0, 0)

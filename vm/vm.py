...
class Node:
	...
	def __nonceifyParty(self, voter_hash, party):
		A = rsa.sign('{}:{}'.format(voter_hash, party), self.getKeys()[1])
		A = hashlib.sha256(json.dumps(A).encode('utf-8')).hexdigest()
		A = '{}${}'.format(party, A)
		return rsa.encrypt(A, BLO_PUBLIC_KEY), rsa.encrypt(A, EXT_OFFICER_PUBLIC_KEY)
		
	def verifyVoterId(self, voter_id):
		DB_MANAGER = ssql.SSQLiteManager('public_details.db')
		q = DB_MANAGER.select('ELIGIBLE_VOTERS', cond = '"voter id" == "{}"'.format(voter_id))
		DB_MANAGER.close()
		if not q:
			return False, 'LOCALLY NOT FOUND'
		else:
			return (True, 'LOCALLY FOUND') if q[0][1] == 1 else (False, 'ALREADY VOTED')
